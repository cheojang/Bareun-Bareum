...
export default function TherapistMessagesPage() {
  ...
  function loadMessages(childId: string) {
    setLoading(true);
    fetch(`/api/therapist/messages?childId=${childId}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  async function sendMessage() {
    ...
    setSending(true);
    const res = await fetch("/api/therapist/messages", {
      method: "POST",
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-57px-56px)] md:h-[calc(100dvh-57px)] px-0 max-w-2xl mx-auto">
      {/* 상단 */}
      <div className="px-5 pt-6 pb-3 md:px-8 space-y-3 border-b border-[#F0E8E0]">
        <div>
          <h1 className="text-2xl font-black text-[#3D3530]">메시지</h1>
          <p className="text-sm text-[#8B7E74] mt-0.5">보호자와 소통하세요</p>
        </div>
        {children.length > 0 && (
          <select
          ...
          >
            {children.map((c) => (
              <option key={c.id} value={c.id}>{c.name} 보호자</option>
            ))}
          </select>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 md:px-8 py-4 space-y-3">
        {loading ? (
          ...
          <div className="text-center pt-12">
            ...
            <p className="text-sm text-[#8B7E74]">{childName} 보호자와의 대화를 시작하세요</p>
          </div>
        ) : (
          messages.map((m) => {
            const isMe = m.senderRole === "therapist";
            return (
              <div
                key={m.id}
