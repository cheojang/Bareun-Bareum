...
function RecordDocument({ record, childName }: { record: RecordData; childName: string }) {
  let recWords: string[] = [];
  try {
    recWords = record.geminiFeedback?.recommendedWords
      ? JSON.parse(record.geminiFeedback.recommendedWords)
      : [];
  } catch {}

  const wordCards = recWords.filter((w) => WORD_IMAGE_SLUGS[w]).slice(0, 10);
  const steps = record.geminiFeedback
    ? [
        record.geminiFeedback.trainingStep1,
        record.geminiFeedback.trainingStep2,
        record.geminiFeedback.trainingStep3,
        record.geminiFeedback.trainingStep4,
      ]
    : [];
  ...
  return (
    <>
      ...
      <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: "16px" }}>
        ...
        <section>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px" }}>
            <div style={{ background: "#FFF5EE", borderRadius: "12px", padding: "12px 16px", textAlign: "center" }}>
              ...
              <div style={{ fontSize: "24px", fontWeight: 900, color: "#3D3530" }}>{record.targetWord}</div>
            </div>
            <div style={{ background: "#FEF2F2", borderRadius: "12px", padding: "12px 16px", textAlign: "center" }}>
              ...
              <div style={{ fontSize: "24px", fontWeight: 900, color: "#EF4444" }}>{record.childPronunciation}</div>
            </div>
            <div style={{ background: "#EDE9FE", borderRadius: "12px", padding: "12px 16px", textAlign: "center" }}>
              ...
              <div style={{ fontSize: "16px", fontWeight: 800, color: "#6D28D9" }}>{record.errorPattern}</div>
              <div style={{ fontSize: "10px", color: "#8B7E74", marginTop: "2px" }}>{record.errorCategory}</div>
            </div>
          </div>
          {record.geminiFeedback?.rootCause && (
            <div style={{ marginTop: "10px", background: "#F8FAFF", borderRadius: "12px", padding: "12px 16px", borderLeft: "3px solid #A8D8CF" }}>
              ...
              <div style={{ fontSize: "11px", color: "#3D3530", lineHeight: 1.6 }}>{record.geminiFeedback.rootCause}</div>
            </div>
          )}
        </section>
        ...
        {/* 부모님께 한 마디 */}
        {record.geminiFeedback?.parentMessage && (
          <section>
            <div style={{ background: "linear-gradient(135deg, #FFF5EE 0%, #F0FAF8 100%)", borderRadius: "12px", padding: "14px 16px", border: "1px dashed #FFB38A" }}>
              ...
              <div style={{ fontSize: "11px", color: "#3D3530", lineHeight: 1.7 }}>{record.geminiFeedback.parentMessage}</div>
            </div>
          </section>
        )}
      </div>
...
export function BulkPrintClient({ childName, records }: Props) {
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      for (let i = 0; i < contentRefs.current.length; i++) {
        ...
        const finalW = (canvas.width / 3.7795) * ratio;
        const finalH = (canvas.height / 3.7795) * ratio;
      ...
      pdf.save(`${childName}_발음훈련지_${records.length}개.pdf`);
    } finally {
      setGenerating(false);
    }
  }
  ...
  return (
    <>
      ...
      <div className="no-print sticky top-0 z-50 bg-[#3D3530] text-white flex items-center justify-center gap-3 py-2.5 px-4">
        <span className="text-sm text-white/70 mr-2">{records.length}개 발음 훈련지 미리보기</span>
        <button
        ...
        >
          {generating ? "생성 중..." : `⬇ PDF 저장 (${records.length}장)`}
        </button>
        <button
      ...
      {/* A4 미리보기 — 레코드별 분리 */}
      <div className="no-print bg-[#F5F0EB] min-h-screen py-8 px-4 flex flex-col items-center gap-8">
