 ...
 * 공유 클라이언트(gemini-client) 사용 — Vertex AI(크레딧) 우선, AI Studio 폴백
 * 서버 환경에서만 실행
 */

import { PhonemeError } from '@/types/phonetics';
import { callWithFallback, withFastConfig, getGenAI, isGeminiConfigured } from './gemini-client';

const SYSTEM_INSTRUCTION = `당신은 아동 언어치료 전문가입니다.
한국 아동의 조음(발음) 오류를 부모에게 쉽고 친근하게 설명하고, 집에서 연습할 수 있는 구체적이고 재미있는 방법을 안내합니다.
- 전문 용어 대신 쉽운 말을 사용하세요
...
{
  "rootCause": "아이가 왜 이런 발음을 했는지 부모가 이해하기 쉽은 1-2문장",
  ...
  "geminiConfidence": 4
    ...
    const responseText = await callWithFallback('Gemini Guidance', async (modelName) => {
      const result = await ai.models.generateContent({
        ...
        config: withFastConfig(modelName, { responseMimeType: 'application/json', systemInstruction: SYSTEM_INSTRUCTION }),
      });
      return result.text ?? '';
    });
    try {
...
이번 연습 전에 부모에게 오늘 집중할 포인트와 격려 메시지를 2문장으로 주세요.`
    ...
    return await callWithFallback('Gemini Briefing', async (modelName) => {
      const result = await ai.models.generateContent({
        model: modelName,
        contents: [{ role: 'user', parts: [{ text: prompt }] }],
        config: withFastConfig(modelName, { systemInstruction: BRIEFING_SYSTEM_INSTRUCTION }),
      });
      return result.text || buildFallbackBriefing(topErrorPhonemes);
    });
  } catch (error) {
...
function buildFallbackBriefing(topErrorPhonemes: string[]): string {
  ...
  const tipMap: Record<string, string> = {
    'ㄹ': `오늘은 '${phoneme}' 소리에 집중해봐요! 혀 끕을 윗니 뒤쪽에 살짝 대고 '라라라' 노래를 불러보면 좋아요.`,
    'ㅅ': `'${phoneme}' 소리가 오늘의 목표예요! 이를 살짝 붙이고 바람을 조금씩 내보내는 연습을 해봐요.`,
    'ㅈ': `오늘은 '${phoneme}' 소리를 집중 연습해요! 혀를 윗잇몸에 던다가 떼면서 '자' 소리를 내봐요.`,
    'ㅊ': `'${phoneme}' 소리가 오늘의 포인트예요! 강하게 바람을 내뱉으며 '차!' 라고 해봐요.`,
    'ㄱ': `'${phoneme}' 소리를 연습할게요! 목 뒤쪽에서 소리가 나도록 '꿀꺽' 삼키는 느낌으로 해봐요.`,
    'ㄴ': `'${phoneme}' 소리에 집중해요! 혀 끕을 윗잇몸에 붙이고 콧소리로 '은~' 해봐요.`,
    'ㅂ': `오늘은 '${phoneme}' 소리를 연습해요! 입술을 붙였다가 '팭!' 하고 터트려봐요.`,
  };

  const tip = tipMap[phoneme] ?? `'${phoneme}' 소리를 집중적으로 연습해볼게요! 천천히, 여러 번 반복하면 꿈 늘어나요.`;
...
function buildFallbackGuidance(errors: PhonemeError[]): {
  ...
  const tips: Record<string, string> = {
    'ㄹ': "혀 끕을 윗니 바로 뒤쪽(입천장)에 살짝 대고 퉹겨보세요.",
    'ㅅ': "윗니와 아랫니를 가깝게 모으고, 혀가 이빨 밖으로 나오지 않게 바람을 '스~' 하고 내보내세요.",
    'ㅈ': "혀를 윗잇몸에 붙였다가 떼어내며 가볝게 '자' 소리를 내보세요.",
    'ㅊ': "'ㅈ' 소리보다 훨씬 더 바람을 세게, '차!' 하고 강하게 터뜨리듯 내보내세요.",
    'ㄱ': "혀뿌리를 들어올려 목구먍 쪽을 막았다가 떼면서 소리를 내보세요.",
    'ㄲ': "일반적인 'ㄱ'보다 목에 힘을 꿉 주고 단단하게 터뜨리는 소리예요.",
    'ㄴ': "혀 끕을 윗잇몸에 붙이고 콧소리로 '은~' 해보세요.",
    'ㅂ': "입술을 붙였다가 '팭!' 하고 터트려보세요.",
  };
  ...
  return {
    ...
    parentMessage: '모든 아이는 자신의 속도로 발달합니다. 매일 5-10분 정도 즐각게 함께 연습하는 것이 가장 효과적입니다!',
    geminiConfidence: 2,
  };
}
