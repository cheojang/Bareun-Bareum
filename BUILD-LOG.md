...
## 🗺️ 버전 2 로드맵 (예정 — 지금 진행 안 함)

### 🌏 일본어 시장 확장 (v2 우선 타깃)

**결정(2026-06-13):** 해외 확장의 **첫 타깃을 일본어로 확정.** 영어는 그 다음(스케일 플레이).

**왜 일본어 먼저인가:**
- 가나=모라 단위 음절문자 → 한글 음절블록 모델과 거의 동일 (영어처럼 철자≠발음 문제 없음)
- 개음절·자음군 거의 없음 → 오류 경우의 수가 작아 단어/패턴 DB 구축이 가벼움
- 일본 소아 조음치료 주요 타깃(さ행·**ら행**·か행·つ) 중 **ら행 유음은 한국어 ㄹ과 동일 문제** → 조음 단면도·훈련 로직 부분 재사용
- 일본어 니치는 영어권 대비 경쟁 낮음, 교육비·앱 결제 문화 강함

**선행 작업(엔진 추상화):** `jamo-analysis.ts`를 언어별 `PhonemeAnalyzer` 인터페이스로 분리 → 일본어(가나/로마자) 분석기를 끼워넣고, 영어(IPA/G2P 발음사전)도 추후 같은 자리에 추가. 이렇게 하면 영어 확장 시 재작업 없음.

**언어 종속(새로 구축 필요):** 단어 DB, 오류 패턴 목록(AI-LOGIC), 조음 단면도, 훈련 팁 — 전부 일본어 전용으로 신규.
**언어 무관(재사용):** 임상 분류 체계(대치·탈락·첨가·왜곡), 4단계 훈련법, 청각폭격, 최소대립쌍, 50회 반복 운동학습, 적응형 난이도, 전체 UX 구조.
...
### 🔥 다음 작업 세션 시작 시 가장 먼저 — DB 마이그레이션 (PC 필요)
- [ ] `git pull origin main` 후 `npx prisma db push` 실행
- 이유: **UserConsent 테이블 생성** (약관 동의 일시 기록 — PR #12, 2026-06-12)
- 실행 전까지는 안전장치로 앱 정상 동작하지만 **동의 수집이 보류됨**
- 실행 후: 로그인 → 동의 기록 없는 회원에게 /consent 1회 표시 → DB 기록 확인
